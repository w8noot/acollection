...
describe("Trade token with whitelist", async () => {
  ...
  it("fulfill order no whitelist", async () => {
    ...
    await expect(tx).to.revertedWith(
      "Mark3dExchange: collection doesn't have whitelist"
    );
  });
  ...
  it("whitelist deadline exceeds", async () => {
    ...
    await expect(tx).to.revertedWith(
      "Mark3dExchange: whitelist deadline exceeds"
    );
  });
  ...
  it("invalid signature", async () => {
    ...
    await expect(tx).to.revertedWith(
      "Mark3dExchange: whitelist invalid signature"
    );
  });
  ...
  it("fullfill whitelist wrong price", async () => {
    ...
    await expect(tx).to.revertedWith(
      "Mark3dExchange: value must equal price with discount"
    );
  });
